...
class Teamlink extends React.Component {
    ...
    getTeam() {
        let proba = this.props.probni
        console.log(proba)
        var url = "http://ergast.com/api/f1/2013/constructors/" + proba + "/constructorStandings.json";
        $.get(url, (data) => {
    ...
    render() {
        ...
        //////////////////////////////////////////
        let gm = this.state.team[0].Constructor
        gm.novi = "nova osobina"
        console.log(gm)
        console.log(drzave[0].country)

        let sss = this.state.teamResul.map((rezultat) => {
            let eee = rezultat.Circuit.Location.country;
            console.log(eee)
            console.log(drzave.filter(drzava => drzava.country === rezultat.Circuit.Location.country)[0].code)
            rezultat.Circuit.Location.code = drzave.filter(drzava => drzava.country === rezultat.Circuit.Location.country)[0].code
            console.log(rezultat)
        })
        console.log(sss)
        console.log(this.state.teamResul)
        ...
        console.log(this.state.team[0].Constructor.constructorId)
        return (
            <div className="centralni">
                <div className="teamlevi">
                    <div className="teamlogo">
                        <div>
                            <img src={constrLogoUrl[0].url} alt="Team logo" width="100" height="100" />
                        </div>
                        <div>
